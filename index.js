	...
	if (arg[0].slice(0, PREFIX.length) == PREFIX) {
		...
		for (var i in Command) {
			if (
				...
				Command[i].alias.includes(command)
			) {
				...
				if (
					Command[i].roleRequirement != 0 &&
					Command[i].roleRequirement.filter((value) =>
						message.member._roles.includes(value)
					).length == 0
				) {
					result = 2;
				} else if (
					Command[i].channelRequirement.length != 0 &&
					!Command[i].channelRequirement.includes(message.channelId)
				) {
					result = 3;
				} else {
        ...
        */
		...
		if (result == -1 || result == 0) {
			...
			message.channel.send("That command cannot be use in this channel!");
	}
});
